...
const StudentProfilePage: React.FC = () => {
            ...
            <div className="bg-white p-6 rounded-lg shadow-md">
                ...
                <div className="flex flex-col md:flex-row items-start space-y-6 md:space-y-0 md:space-x-8">
                    <div className="flex-shrink-0">
                        <div className="w-32 h-32 rounded-full border-2 border-brand-primary/30 flex items-center justify-center bg-slate-50">
                            {user.profilePictureUrl ? (
                                <img src={user.profilePictureUrl} alt="Profile" className="w-full h-full object-cover rounded-full" />
                            ) : (
                                <span className="text-4xl font-bold text-brand-primary">{user.name.split(' ').map(n => n[0]).join('').substring(0, 2)}</span>
                            )}
                        </div>
                    </div>
                    <div className="w-full space-y-4">
                        <h3 className="text-3xl font-bold text-slate-800">{user.name}</h3>
                        <p className="text-slate-600 max-w-2xl">{user.bio || 'Bio belum diisi.'}</p>
    ...
    return (
        <div className="bg-white p-6 rounded-lg shadow-md">
            <h2 className="text-2xl font-bold mb-6 text-slate-800">Edit Profil</h2>
            <div className="space-y-6">
                <div className="flex items-center space-x-6">
                    <div className="w-24 h-24 rounded-full border-2 border-brand-primary/30 flex items-center justify-center bg-slate-50">
                        {formData.profilePictureUrl ? (
                            <img src={formData.profilePictureUrl} alt="Profile" className="w-full h-full object-cover rounded-full" />
                        ) : (
                            <span className="text-3xl font-bold text-brand-primary">{user.name.split(' ').map(n => n[0]).join('').substring(0, 2)}</span>
                        )}
                    </div>
                    <div>
                         <label htmlFor="picture-upload" className="cursor-pointer bg-white py-2 px-4 border border-slate-300 rounded-md shadow-sm text-sm font-medium text-slate-700 hover:bg-slate-50 flex items-center">
                            <UploadIcon />
                            <span>Ubah Foto</span>
                            <input id="picture-upload" name="picture-upload" type="file" className="sr-only" onChange={handlePictureChange} accept="image/*" />
                        </label>
                        <p className="text-xs text-slate-500 mt-2">JPG, PNG, atau GIF. Maks 800x800px.</p>
                    </div>
                </div>
                <div>
                 ...
                 <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                        <label className="block text-sm font-medium text-slate-700">Jurusan</label>
                ...
                <div>
                    <label className="block text-sm font-medium text-slate-700">Keahlian (pisahkan dengan koma)</label>
                    <input type="text" name="skills" value={formData.skills} onChange={handleChange} className={formInputStyle} />
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        <label className="block text-sm font-medium text-slate-700">Link CV (Google Drive, dll)</label>
                        <input type="url" name="cvLink" value={formData.cvLink} onChange={handleChange} className={formInputStyle} />
                    </div>
                    <div>
                        <label className="block text-sm font-medium text-slate-700">Link Portofolio</label>
