        ...
        <ol role="list" className="space-y-4 md:flex md:space-x-8 md:space-y-0">
            {STEPS.map((step) => (
                <li key={step.name} className="md:flex-1">
                    {currentStep > step.id ? (
                        <div className="group w-full border-l-4 border-brand-primary py-2 pl-4 transition-colors md:border-l-0 md:border-t-4 md:pb-0 md:pl-0 md:pt-4">
                            <span className="text-sm font-medium text-brand-primary">{step.name}</span>
                        </div>
                    ) : currentStep === step.id ? (
                        <div className="w-full border-l-4 border-brand-primary py-2 pl-4 md:border-l-0 md:border-t-4 md:pb-0 md:pl-0 md:pt-4" aria-current="step">
                            <span className="text-sm font-bold text-brand-primary">{step.name}</span>
                        </div>
                    ) : (
...
const FileInput: React.FC<{
    ...
    <div>
        ...
        <label htmlFor={id} className="mt-1 flex items-center justify-center px-4 py-2 border border-gray-300 rounded-md shadow-sm text-sm font-medium text-slate-700 bg-white hover:bg-gray-50 cursor-pointer transition-colors">
        ...
        {file && <p className="text-sm text-brand-primary mt-2 font-medium">File terpilih: {file.name}</p>}
...
const ApplicationFormPage: React.FC = () => {
    ...
    const formInputStyle = (field: string) => `mt-1 block w-full border rounded-lg shadow-sm py-2.5 px-3 bg-white text-slate-800 focus:outline-none focus:ring-2 focus:ring-brand-primary transition-shadow ${errors[field] ? 'border-red-500' : 'border-slate-200'}`;
    const formSelectStyle = (field: string) => `mt-1 block w-full border rounded-lg shadow-sm py-2.5 px-3 bg-white text-slate-800 focus:outline-none focus:ring-2 focus:ring-brand-primary transition-shadow ${errors[field] ? 'border-red-500' : 'border-slate-200'}`;
    const formReadonlyStyle = "mt-1 block w-full bg-slate-50 border border-slate-200 rounded-lg py-2.5 px-3 text-slate-600 font-medium";
    
    return (
        <div className="container mx-auto px-6 py-8 animate-fade-in-slide-up">
            <Link to="/student" className="inline-flex items-center text-brand-primary hover:text-brand-dark font-medium mb-6 transition-colors">
                <ArrowLeftIcon />
                Kembali ke Pencarian
            </Link>
            <div className="bg-white p-8 rounded-2xl shadow-soft border border-slate-100 max-w-4xl mx-auto">
                <div className="text-center pb-4 mb-8 border-b border-slate-100">
                    <h1 className="text-3xl font-heading font-bold text-slate-900">Formulir Lamaran</h1>
                    <p className="text-lg text-slate-600 mt-2 font-medium">{internship.title} <span className="text-slate-300 mx-2">|</span> <span className="text-brand-primary">{internship.companyName}</span></p>
                </div>
                ...
                <div className="mb-10">
                 ...
                 {hasApplied ? (
                     <div className="text-center py-10 bg-brand-light/30 border border-brand-light rounded-xl">
                        <p className="text-lg font-semibold text-brand-primary">Anda sudah melamar untuk posisi ini.</p>
                        <p className="text-slate-600 mt-1">Anda dapat melihat status lamaran Anda di halaman "Lamaran Saya".</p>
                    </div>
                ) : (
                <form onSubmit={handleSubmit} className="space-y-8">
                    {/* Render Step Content */}
                    <div className="p-6 border border-slate-200 rounded-xl bg-white shadow-sm min-h-[300px]">
                        ...
                        {currentStep === 3 && (
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div><label htmlFor="gpa" className={labelStyle}>IPK Terakhir <span className="text-red-500">*</span></label><input type="number" id="gpa" name="gpa" step="0.01" min="0" max="4" value={formData.gpa === undefined ? '' : formData.gpa} onChange={handleChange} className={formInputStyle('gpa')} required />{errors.gpa && <p className="text-red-500 text-xs mt-1">{errors.gpa}</p>}</div>
                        ...
                        {currentStep === 4 && (
                            <div className="space-y-6">
                                ...
                                <div><label htmlFor="expectedSalary" className={labelStyle}>Gaji/Insentif Diharapkan (Opsional)</label><input type="number" id="expectedSalary" name="expectedSalary" value={formData.expectedSalary === undefined ? '' : formData.expectedSalary} onChange={handleChange} className={formInputStyle('expectedSalary')} /></div>
                        ...
                        {currentStep === 5 && (
                            <div className="space-y-6">
                                <div className="bg-brand-light/20 p-6 rounded-xl border-l-4 border-brand-primary">
                                    <h4 className="font-bold text-brand-dark text-lg mb-2">Tinjau Kembali Lamaran Anda</h4>
                                    <p className="text-slate-600">Pastikan semua data dan dokumen yang Anda masukkan sudah benar sebelum mengirimkan lamaran. Lamaran yang sudah dikirim tidak dapat diubah.</p>
                                </div>
                                <div className="flex items-start p-4 bg-white border border-slate-200 rounded-lg hover:border-brand-primary/50 transition-colors">
                                    <input id="dataAuthenticityConfirmation" name="dataAuthenticityConfirmation" type="checkbox" checked={!!formData.dataAuthenticityConfirmation} onChange={handleChange} className="h-5 w-5 text-brand-primary focus:ring-brand-primary border-gray-300 rounded mt-0.5" />
                                    <div className="ml-3">
                                        <label htmlFor="dataAuthenticityConfirmation" className="font-medium text-slate-800 cursor-pointer">Saya menyatakan bahwa seluruh data yang saya isi adalah benar dan dapat dipertanggungjawabkan.</label>
                                        {errors.dataAuthenticityConfirmation && <p className="text-red-500 text-xs mt-1">{errors.dataAuthenticityConfirmation}</p>}
                                    </div>
                                </div>
                                <div className="flex items-start p-4 bg-white border border-slate-200 rounded-lg hover:border-brand-primary/50 transition-colors">
                                    <input id="dataProcessingConsent" name="dataProcessingConsent" type="checkbox" checked={!!formData.dataProcessingConsent} onChange={handleChange} className="h-5 w-5 text-brand-primary focus:ring-brand-primary border-gray-300 rounded mt-0.5" />
                                    <div className="ml-3">
                                        <label htmlFor="dataProcessingConsent" className="font-medium text-slate-800 cursor-pointer">Saya menyetujui data saya diproses dan dibagikan kepada perusahaan terkait untuk keperluan seleksi.</label>
                                        {errors.dataProcessingConsent && <p className="text-red-500 text-xs mt-1">{errors.dataProcessingConsent}</p>}
                                    </div>
                                </div>
                                <div>
                                    <label htmlFor="eSignature" className="block text-sm font-bold text-slate-700 mb-2">Tanda Tangan Digital (Ketik Nama Lengkap) <span className="text-red-500">*</span></label>
                                    <input type="text" id="eSignature" name="eSignature" value={formData.eSignature || ''} onChange={handleChange} placeholder="Ketik nama lengkap Anda di sini" className={`${formInputStyle('eSignature')} font-serif italic text-lg`} required />
                    ...
                    <div className="flex justify-between items-center pt-4">
                        <button type="button" onClick={handlePrev} disabled={currentStep === 1} className="px-6 py-3 bg-white text-slate-700 font-semibold rounded-xl border border-slate-300 hover:bg-slate-50 hover:text-slate-900 disabled:opacity-50 disabled:cursor-not-allowed transition-all">
                            Sebelumnya
                        </button>
                        {currentStep < STEPS.length ? (
                            <button type="button" onClick={handleNext} className="px-8 py-3 bg-brand-primary text-white font-bold rounded-xl hover:bg-brand-dark shadow-lg shadow-brand-primary/25 hover:-translate-y-0.5 transition-all">
                                Selanjutnya
                            </button>
                        ) : (
                            <button type="submit" className="px-8 py-3 bg-brand-teal text-slate-900 font-bold rounded-xl hover:bg-teal-300 shadow-lg hover:-translate-y-0.5 transition-all">
                                Kirim Lamaran
                            </button>
                        )}
                    </div>
