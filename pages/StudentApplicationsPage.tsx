...
const StudentApplicationsPage: React.FC = () => {
    ...
    return (
        <div className="bg-white p-6 rounded-lg shadow-md">
            <h2 className="text-2xl font-bold mb-6 text-slate-800">Lamaran Saya</h2>
            {myApplications.length > 0 ? (
                <div className="space-y-4">
                    {myApplications.map(app => (
                        <div key={app.id} className="p-4 border rounded-md flex flex-col md:flex-row justify-between items-start md:items-center bg-slate-50">
                            <div className="flex-grow">
                                <h3 className="font-bold text-slate-800">{app.internship?.title}</h3>
                                <p className="text-sm text-slate-600">{app.internship?.companyName}</p>
                ...
                <p className="text-center text-slate-500 py-8">Anda belum melamar internship apapun.</p>
            )}
        </div>
    );
};
